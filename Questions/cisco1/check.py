...
class Link:
    ...
    def dot(self, showip):
        ...
        if self.hide_port == False:
            s += ',taillabel="%s",headlabel="%s"'%(
                self.tail(showip), self.head(showip),
                )
        if self.hide_port is None: # Cache le port destination
            s += ',taillabel="%s"' % self.tail(showip)
...
class Network:
    def __init__(self, dirname, display_eth1=False, display_cisco_eth1=False):
        self.dirname = dirname
        self.hosts = {}
        Network.hosts = self.hosts # XXX Horrible XXX
        self.cache_nr_networks = None
        self.display_eth1 = display_eth1
        self.display_cisco_eth1 = display_cisco_eth1
...
""" % (os.path.join(self.dirname, 'HTML'), how, filename, font1, font2,
    ...
    def html(self, from_node):
        ...
        for h in hosts:
            ...
            x += [h.C0.remote_port.host.name, e0]
            if self.display_cisco_eth1:
                x.append(h.C0.remote_port.host.E1.port_ip())
            x += [s0, s1]
            for v in x:
                s.append("<td>%s</td>" % v)
            s.append("</tr>")
...
postes = (
["10.246.72.201", 'A-01',Cisco2800],
["10.246.72.202", 'A-02',Cisco1800], # SB3 Cisco2960
("10.246.72.203", 'A-03',Cisco2900),
("10.246.72.204", 'A-04',Cisco1800), # SD3 Cisco2950
("10.246.72.205", 'B-05',Cisco2800B),
("10.246.72.206", 'B-06',Cisco1800), # SF3 Cisco2950
("10.246.72.207", 'B-07',Cisco2900),
("10.246.72.208", 'B-08',Cisco1800), # SH3 Cisco2960
("10.246.72.209", 'C-09',Cisco2800),
("10.246.72.210", 'C-10',Cisco1800), # SJ3 Cisco2960
("10.246.72.211", 'C-11',Cisco2900),
("10.246.72.212", 'C-12',Cisco1800), # SL3 Cisco2950
("10.246.72.213", 'D-13',Cisco2800B),
("10.246.72.214", 'D-14',Cisco1800), # SN3 Cisco2950
("10.246.72.215", 'D-15',Cisco2900),
("10.246.72.216", 'D-16',Cisco1800), # SP3 Cisco2950
)
